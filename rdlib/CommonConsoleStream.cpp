...
//https://github.com/PazerOP/DemoLib2/blob/master/CommonConsole/CommonConsoleStream.cpp
namespace cc
{
	...
	static void ClearLine()
	{
		...
		DWORD dummy;
		if (!FillConsoleOutputCharacterA(conHandle, ' ', cInfo.dwSize.X - cInfo.dwCursorPosition.X,
		                                 cInfo.dwCursorPosition, &dummy))
			throw std::runtime_error("Failed to FillConsoleOutputCharacter");
	...
	void CommonConsoleStream::ApplyEscapeSequence(const std::string_view& escSeq)
	{
		...
		int x, y;
		if ((sscanf_s(escSeq.data(), "[%i;%iH%n", &x, &y, &pos) == 2 || sscanf_s(
			escSeq.data(), "[%i;%if%n", &x, &y, &pos) == 2) && pos == (escSeq.size() - 1)) {
			SetCursorPos(x, y);

			return;
		}
		int baseSetting;
		if (sscanf_s(escSeq.data(), "[%i%n", &baseSetting, &pos) == 1 && pos == (escSeq.size() - 1)) {
