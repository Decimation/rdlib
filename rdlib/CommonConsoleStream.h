...
// https://github.com/PazerOP/DemoLib2/blob/master/CommonConsole/CommonConsoleStream.hpp
namespace cc
{
...
#define OSTR_MANIP(name) template<class _Elem, class _Traits> inline std::basic_ostream<_Elem, _Traits>& name(std::basic_ostream<_Elem, _Traits>& os)
	...
	namespace curpos
	{
		struct moveleft : impl::move
		{
			constexpr moveleft(int delta) : move(delta)
			{
			}
		};

		struct moveright : impl::move
		{
			constexpr moveright(int delta) : move(delta)
			{
			}
		};

		struct moveup : impl::move
		{
			constexpr moveup(int delta) : move(delta)
			{
			}
		};

		struct movedown : impl::move
		{
			constexpr movedown(int delta) : move(delta)
			{
			}
		};
...
inline std::basic_ostream<_Elem, _Traits>& operator<<(std::basic_ostream<_Elem, _Traits>& os, const cc::fg::rgb& rgb)
{
	return os << "\x1B[38;2;" << +rgb.r << ';' << +rgb.g << ';' << +rgb.b << 'm';
}

template <class _Elem, class _Traits>
inline std::basic_ostream<_Elem, _Traits>& operator<<(std::basic_ostream<_Elem, _Traits>& os, const cc::bg::rgb& rgb)
{
	return os << "\x1B[48;2;" << +rgb.r << ';' << +rgb.g << ';' << +rgb.b << 'm';
}

template <class _Elem, class _Traits>
inline std::basic_ostream<_Elem, _Traits>& operator<<(std::basic_ostream<_Elem, _Traits>& os,
                                                      const cc::curpos::moveup& up)
{
...
template <class _Elem, class _Traits>
inline std::basic_ostream<_Elem, _Traits>& operator<<(std::basic_ostream<_Elem, _Traits>& os,
                                                      const cc::curpos::moveright& right)
{
